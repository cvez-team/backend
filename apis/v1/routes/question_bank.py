...
async def get_question_bank(project_id: str, position_id: str, question_id: str, user: Annotated[UserSchema, Depends(get_current_user)]):
    question_bank = get_current_question_bank(project_id, position_id, user)
...
async def create_question_bank(project_id: str, position_id: str, user: Annotated[UserSchema, Depends(get_current_user)]):
    question_bank = create_new_question_bank(project_id, position_id, user)
    return jsonResponseFmt(question_bank.to_dict())


@router.put("/{project_id}/{position_id}/{question_bank_id}", response_model=QuestionBankResponseInterface)
async def update_question_bank(project_id: str, position_id: str, question_id: str, user: Annotated[UserSchema, Depends(get_current_user)]):
    update_current_question_bank(project_id, position_id, question_id, user)
...
async def delete_question(project_id: str, position_id: str, question_id: str, user: Annotated[UserSchema, Depends(get_current_user)]):
    delete_current_question_bank(project_id, position_id, question_id, user)
    return jsonResponseFmt(None, f"Question Bank with ID {question_id} has been deleted successfully.")
